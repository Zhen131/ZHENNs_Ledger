...
| `golden-backup-format-v3-ledger-schema-v5-time-zone.json` | backup format 3; ledger schema 5 | 2026-09-06 | Fictional schema 5 envelope covering all four fact types. Trade, asset transfer, and price snapshot record IANA time zones, each with the offset that location really had at that instant; the cash event omits the optional field. | — |
| `golden-ledger-file-format-v3-crypto-v1-ledger-schema-v5-time-zone.lftl` | file format 3; crypto 1; ledger schema 5; backup format 3 | 2026-09-06 (regenerated 2026-09-07) | Fictional chunked C-file generated through the real repository create path from `createGoldenStorageScenario`. It covers all four fact types, with the same optional time-zone presence and absence as the V5 B fixture. Every recorded location carries the offset that location really had at that instant: `Asia/Shanghai` with `+08:00` and `Europe/Budapest` with its summer `+02:00`. | `W16-Golden-V5-Fictional-Ledger` |
