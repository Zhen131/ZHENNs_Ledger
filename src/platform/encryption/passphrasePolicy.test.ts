...
describe("passphrase policy", () => {
  it("accepts 8 through 128 Unicode code points", () => {
    expect(validatePassphrase("a".repeat(8))).toEqual({ ok: true });
    expect(validatePassphrase("🔐".repeat(8))).toEqual({ ok: true });
    expect(validatePassphrase("账本🔐safe8")).toEqual({ ok: true });
    expect(validatePassphrase("a".repeat(128))).toEqual({ ok: true });
  });

  it("rejects values outside the exact code-point limits", () => {
    expect(validatePassphrase("a".repeat(7)).ok).toBe(false);
