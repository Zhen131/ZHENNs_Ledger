...
- 大账本性能已有实测基线，但优化尚未开始；当前不宣称任意规模都流畅
...
- 尚未实现分页与虚拟列表。已用合成账本量出改动前的性能基线：production 模式下 10⁴ 笔档位单字符输入约 1.5 秒、页面切换约 1.7～2.4 秒、冷启动约 5.7 秒；10⁵ 笔档位的明文备份约 69 MB，先被产品 8 MiB 导入资源上限拒绝，尚未进入渲染；10⁶ 笔档位在明文备份序列化阶段即触及 JavaScript 字符串上限。优化尚未开始
