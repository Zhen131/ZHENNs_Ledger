...
- 大账本性能已完成第一轮优化（不重复算），10⁴ 笔档位的计算与输入已达标；但交易页仍是一次画完全部行，10⁵ 笔起页面切换仍不可用。当前不宣称任意规模都流畅
...
- 尚未实现分页与虚拟列表：交易页有多少条流水就画多少行。已完成第一轮优化「不重复算」（派生结果缓存、写入前单次扫描、输入草稿隔离、增量派生更新），production 模式下 10⁴ 笔档位的实测变化如下：

  | 指标 | 优化前 | 当前 |
  | --- | ---: | ---: |
  | 单字符输入 | 约 1,535 ms | 约 75 ms |
  | 页面切换 | 1,686～2,409 ms | 370～1,143 ms |
  | 冷启动 | 约 5,710 ms | 约 1,958 ms |
  | 记一笔 | 约 7,652 ms | 约 2,624 ms |

  资源上限已随之放宽（C 文件外壳 384 MiB、导入资源策略 128 MiB），10⁵ 笔档位因此可以导入，但浏览器在交易页仍会超时或崩溃；10⁶ 笔档位在明文备份序列化阶段即触及 JavaScript 单条字符串上限，尚未进入渲染。**页面切换的剩余耗时来自始终挂载的大表渲染，第二轮优化正在进行中，尚未合入本分支。**
