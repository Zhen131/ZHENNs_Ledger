...
截至 2026-08-28，开发分支 `zhennn/w15-main-v4-display-home` 已完成 Week 15 第一批的三个功能阶段：资产转入转出与三位置重放、统一只读数字格式化、首页前五持仓成本区，以及饼图／热力图修整。三个阶段提交依次为 `10ca0fe`、`45701d4`、`1e4e3df`；验收阶段又以 `80bb92d` 落实后置版本分层决定，保持备份格式 V3、只升账本 schema V4，并加入冻结黄金样例。真实 Chrome 验收发现 macOS 原生选择器会把自定义 `.lftl` 识别为通用二进制文件，`99bea69` 因此仅修正 picker MIME 提示并保留后缀守卫。本分支尚未合并或推送，不能描述为已经进入 `main`。

最终统一自动质量门为 92 个测试文件、1081 项测试全部通过，typecheck、lint、Next 15.5.22 production build 和差异检查通过。真实 Google Chrome 与 macOS 原生 picker 也已完成四项手工确认：饼图提示框可越出卡片完整展示；低于 2% 的小额资产合并为“其他”并可悬停展开；365 日热力图填满整行；V3 `.lftl` 在密码与解密前明确中文拒绝，原文件不变，之后仍可新建 V4 并进入导入流程。这构成 Week 15 开发执行候选 `PASS`，不等于独立验收通过。
...
- Week 15 ledger schema V4 当前停留在 `zhennn/w15-main-v4-display-home` 开发分支；92 个测试文件／1081 项测试、typecheck、lint、production build 与真实 Chrome 四项手工确认全部通过，构成开发执行候选 `PASS`。它仍未合并、未推送，且不等于独立验收通过。
...
- Week 15 macOS 原生 `.lftl` picker 修正：`99bea69f505960f69866d4b720be85876de9bf7b`
